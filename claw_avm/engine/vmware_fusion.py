"""
Claw VMware Fusion Engine - macOS VMware Fusion 虚拟化引擎实现

基于 VMware Fusion for Mac 的虚拟化管理实现
...
class ClawSecureVMwareFusionEngine(ClawSecureEngine):
    """
    VMware Fusion (macOS) 安全虚拟化引擎
    ...
    def __init__(self, workspace_path: Path, vmrun_path: Optional[str] = None):
        ...
        if platform.system() != 'Darwin':
            logger.warning("VMwareFusionEngine is designed for macOS only")
        
        # vmrun 工具路径
        self.vmrun = vmrun_path or self._find_vmrun()
        
        # VMware 虚拟机目录 (macOS 默认位置)
        ...
        logger.info(f"VMware Fusion Engine initialized at {self.workspace}")
    
    def _find_vmrun(self) -> str:
        """查找 vmrun 可执行文件 (macOS)"""
        # macOS 上 VMware Fusion 的常见安装路径
        possible_paths = [
        ...
        logger.warning("vmrun not found, please install VMware Fusion")
        return "vmrun"  # 默认尝试命令
    ...
    def _generate_vm_identifier(self, name: str) -> str:
        """生成 macOS 风格的虚拟机包名称"""
        vm_id = str(uuid.uuid4())[:8]
        return f"{name}_{vm_id}.vmwarevm"
    
    # ============ 核心方法实现 ============
    
    def create_secure_vm(self, config: VMConfig) -> str:
        """
        创建安全虚拟机 (macOS 版本使用 .vmwarevm 包格式)
        ...
        """
        vm_id = str(uuid.uuid4())[:8]
        vm_bundle_name = f"{config.name}_{vm_id}.vmwarevm"
        
        # macOS 上 VMware Fusion 使用 .vmwarevm 包结构
        vm_path = self.vm_dir / vm_bundle_name
    ...
    def _generate_secure_vmx(self, config: VMConfig, vm_path: Path) -> str:
        """生成安全隔离的 VMX 配置 (macOS 版本)"""
        
        # macOS 上的基础配置 (Darwin 内核)
        vmx_lines = [
            '# ClawAVM Secure VM Configuration for macOS',
    ...
    def clone_vm(self, source_vm_id: str, new_name: str) -> str:
        ...
        new_vm_bundle = f"{new_name}_{new_vm_id}.vmwarevm"
        new_vm_path = self.vm_dir / new_vm_bundle
    ...
    def delete_vm(self, vm_id: str) -> bool:
        ...
        # 从 VMware 注销
        vbox_name = self._registry.get(vm_id, {}).get("vbox_name")
        if vbox_name:
            self._run_vmrun(["unregister", str(vmx_path)])
        
        # 删除 VM 文件 (整个 .vmwarevm 包)
